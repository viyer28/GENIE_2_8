...
TH1D * EffectiveSF::MakeEffectiveSF(const Target & target) const
{
  ...
  if(it != fProbDistParams.end()) {
    ...
    return this->MakeEffectiveSF(v[0],v[1],v[2],v[3],v[4],v[5],v[6],target);
  }
  
  if(target.A() == 2)
    return this->MakeEffectiveSF(0.413475,1.75629,8.29029,0.00362127,0.186987,6.24155,0.000208154,target);
  if(target.A() == 3)
    return this->MakeEffectiveSF(3.05583,0.901861,10.932,6.02976,199.551,1.92379,5.26025e-05,target);
  else if(target.A()>3 && target.A()<8)
    return this->MakeEffectiveSF(2.14131,0.775316,9.72776,7.56679,183.428,5.53273,0.000589888,target);
  else if(target.A()>7 && target.A()<17)
    return this->MakeEffectiveSF(2.11978,0.736561,12.941,10.6195,197.014,9.94218,4.36074e-05,target);
  else if(target.A()>16 && target.A()<26)
    return this->MakeEffectiveSF(1.81985,0.609983,6.81283,6.07869,25.8688,0.590602,0.00220846,target);
  else if(target.A()>25 && target.A()<39)
    return this->MakeEffectiveSF(1.7394,0.620821,7.20039,6.73047,21.0049,0.592854,0.00121452,target);
  else if(target.A()>38 && target.A()<56)
    return this->MakeEffectiveSF(1.66649,0.614772,8.54288,8.61544,199.992,6.24716,0.00269039,target);
  else if(target.A()>55 && target.A()<61)
    return this->MakeEffectiveSF(1.79235,0.597282,7.09555,6.25598,18.3744,0.505288,0.00140556,target);
  else if(target.A()>60)
    return this->MakeEffectiveSF(1.52067,0.585172,11.2361,13.3336,174.424,5.28673,9.27737e-05,target);
  return NULL;
}
//____________________________________________________________________________
TH1D * EffectiveSF::MakeEffectiveSF(double bs, double bp, double alpha, double beta, double c1, double c2, double c3, const Target & target) const
{
  ...
  //-- store
  fProbDistroMap.insert(map<string, TH1D*>::value_type(target.AsString(),prob));
  return prob; 
}
//____________________________________________________________________________
...
void EffectiveSF::LoadConfig(void)
{
  ...
  if(gc->GetBoolDef("UseTransverseEnhancement", false))
    fTransEnh1p1hMod = gc->GetDoubleDef("TransEnhf1p1hMod", 1.23);
