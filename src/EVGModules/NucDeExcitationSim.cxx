...
void NucDeExcitationSim::ArgonTargetSim(GHepRecord * evrec) const
{
	...
	const Int_t numev=1;//Number of events
	TRandom3* r = new TRandom3();

	Double_t vertex[3];//Isotropic vector
	...
	Double_t b[num_start_levels] = {0.9,1.5,0.11,0.06,0.04,0.01,0.16,0.26,0.01,0.05,0.11,0.29,3.84,0.31,0.38,0.47,0.36,0.23,0.03,0.11,0.13};//Transition strengths 

	Double_t w=0;//Energy of electron
	Double_t p=0;//Momentum of electron
	Double_t f=0; //Fermi function
	Double_t level_cs[num_start_levels];//Level cross-section
	Int_t highest_level=0;

	for(Int_t i=0;i<num_start_levels;i++) {//Calculates level cross-section
	...
	while (i<numev) {	//Event loop 
		...
		vertex[0]=(r->Rndm())*256.35;//Vertices of vector
		...
		while(level != ground_level){	//Level loop
			...
			for (int ilevel=0; ilevel < this->BrSize(level); ilevel++) { //Decay loop
				if (rl<(this->BranchingRatio(level,ilevel)+tprob)) {

					// We have a decay

					particle_c++;
					level = this->DecayTo(level,decaynum);
				...
				if((tprob+this->BranchingRatio(level,ilevel))>1) {
					nomoredecay = 1; //If it doesn't do any more gamma decay
					break;
				}

				decaynum++;
				tprob += this->BranchingRatio(level,ilevel);
...
//__________________________________________________________________________
double NucDeExcitationSim::BranchingRatio(int level, int ilevel) const
{
	std::vector<double> br[73];
	// level to decay to 

	std::vector<int> decayto[73];
	...
	double brvalue = br[level][ilevel];

	return brvalue;
}
//__________________________________________________________________________
double NucDeExcitationSim::DecayTo(int level, int decaynum) const
{
	std::vector<double> br[73];
	// level to decay to 
	...
	decayto[71].push_back(72);
        ...
        double decayvalue = decayto[level][decaynum];
	
	return decayvalue;
}
//__________________________________________________________________________
int NucDeExcitationSim::BrSize(int level) const
{
	std::vector<double> br[73];
	// level to decay to 
